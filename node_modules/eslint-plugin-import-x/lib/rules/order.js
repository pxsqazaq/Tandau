...
const log = (0, debug_1.default)('eslint-plugin-import-x:rules:order');
                ...
                if (node.specifiers.length > 0 || options.warnOnUnassignedImports) {
                    ...
                    if (named.import) {
                        makeNamedOrderReport(context, node.specifiers
                            ...
                            .map(specifier => ({
                            node: specifier,
                            value: (0, utils_1.getValue)(specifier.imported),
                            type: 'import',
            ...
            ...(named.export && {
                ExportNamedDeclaration(node) {
                    makeNamedOrderReport(context, node.specifiers.map(specifier => ({
                        node: specifier,
                        value: (0, utils_1.getValue)(specifier.local),
                        type: 'export',
                        kind: specifier.exportKind,
                        ...(specifier.local.range[0] !== specifier.exported.range[0] && {
                            alias: (0, utils_1.getValue)(specifier.exported),
                        }),
                    })));
                },
            }),
