...
import {
  ...
  init_dist,
  test
} from "../chunk-ZE4ZZELD.mjs";
import {
  init_purge_cache,
  purgeCache
} from "../chunk-EZL2F32K.mjs";
import {
  __commonJS,
  __esm,
  __toESM
...
var require_purge_cache_test = __commonJS({
  "src/lib/purge_cache.test.ts"() {
    ...
    var hasFetchAPI = import_semver.default.gte(process2.version, "18.0.0");
    beforeEach(() => {
      delete process2.env.NETLIFY_PURGE_API_TOKEN;
      delete process2.env.SITE_ID;
      delete process2.env.NETLIFY_LOCAL;
    });
    afterEach(() => {
    ...
    test("Calls the purge API endpoint and returns `undefined` if the operation was successful", async () => {
      ...
      process2.env.NETLIFY_PURGE_API_TOKEN = mockToken;
      process2.env.SITE_ID = mockSiteID;
    ...
    test("Does not default the deploy_alias field to process.env.NETLIFY_BRANCH if supplied in the options", async () => {
      ...
      process2.env.NETLIFY_PURGE_API_TOKEN = mockToken;
      process2.env.SITE_ID = mockSiteID;
      process2.env.NETLIFY_BRANCH = "main";
      const mockAPI = new MockFetch().post({
        body: (payload) => {
          const data = JSON.parse(payload);
          globalExpect(data.site_id).toBe(mockSiteID);
    ...
    test("Defaults the deploy_alias field to process.env.NETLIFY_BRANCH if not running locally", async () => {
      ...
      const mockAPI = new MockFetch().post({
        body: (payload) => {
          const data = JSON.parse(payload);
          globalExpect(data.site_id).toBe(mockSiteID);
          globalExpect(data.deploy_alias).toBe(process2.env.NETLIFY_BRANCH);
        },
        headers: { Authorization: `Bearer ${mockToken}` },
        method: "post",
        response: new Response(null, { status: 202 }),
    ...
    test("Does not default the deploy_alias field to process.env.NETLIFY_BRANCH when running locally", async () => {
      ...
      process2.env.NETLIFY_PURGE_API_TOKEN = mockToken;
      process2.env.SITE_ID = mockSiteID;
      process2.env.NETLIFY_LOCAL = "true";
      process2.env.NETLIFY_BRANCH = "main";
      const mockAPI = new MockFetch().post({
        body: (payload) => {
          const data = JSON.parse(payload);
...
export default require_purge_cache_test();
